...
function resetIndexHighlights(index){
    const indices = document.querySelectorAll('.index');
    if (!indices[index]) return;
    const highlight = indices[index].querySelector('.index-highlight');
    if (!highlight) return;
...
function animateIndexHighlights(index) {
    const indices = document.querySelectorAll('.index');
    if (!indices[index]) return;
    const highlight = indices[index].querySelector('.index-highlight');
    if (!highlight) return;
...
document.addEventListener("click", () => {
    clearTimeout(storyTimeout);
    resetIndexHighlights(activeStory);
    changeStory();
});
